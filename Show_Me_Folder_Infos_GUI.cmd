...
cls
color 07
title  Show Me Folder Infos GUI v1.0.0
mode 100, 30
...
echo:
echo:
echo:             Welcome to Show Me Folder Infos GUI v1.0.0
echo:
...
if %_erl%==4 setlocal & call :SubFoldersFilesListA    & cls & endlocal & goto :MainMenu
if %_erl%==3 setlocal & call :SubFoldersFilesListR    & cls & endlocal & goto :MainMenu
if %_erl%==2 setlocal & call :ListAllFiles            & cls & endlocal & goto :MainMenu
if %_erl%==1 setlocal & call :ShowAllFiles            & cls & endlocal & goto :MainMenu
goto :MainMenu
...
echo:
echo Total files in current folder and its subfolders: %cnt_files%
echo Total subfolders in current folder (current folder is not counted): %cnt_dirs%
echo The (first) file with the longest name is: %longer_filename% (!longer_filename_lenght! characters long)
echo The (first) path with the longest name is: %longer_pathname% (!longer_pathname_lenght! characters long)
echo The (first) folder with the deepest path is: %deepest_pathfolder% (!deepest_pathfolder_levels! levels)
endlocal
...
set /A deepest_pathfolder_levels=0
set /A len=1
echo Directory %cd%> files_list_info.txt
for %%F in ("*") do (                                               %= Loop through the current directory. =%
  echo    → %%F
  set /A cnt_files+=1
  rem set /A cnt_dirs+=1                                            %= The current directory should not be counted. =%
...
echo:>> files_list_info.txt
echo "Total files in current folder and its subfolders: %cnt_files%">> files_list_info.txt
echo "Total subfolders in current folder (current folder is not counted): %cnt_dirs%">> files_list_info.txt
echo "The (first) file with the longest name is: %longer_filename% (!longer_filename_lenght! characters long)">> files_list_info.txt
echo "The (first) path with the longest name is: %longer_pathname% (!longer_pathname_lenght! characters long)">> files_list_info.txt
echo "The (first) folder with the deepest path is: %deepest_pathfolder% (!deepest_pathfolder_levels! levels)">> files_list_info.txt
endlocal

:: echo/ & pause & cls
rem popd
echo:
echo File "files_list_info.txt" created successfully.
echo Press any key to continue...
pause >nul
popd
exit /b
...
popd
echo:
echo File "files_list_r.txt" created successfully.
echo Press any key to continue...
pause >nul
popd
exit /b
...
popd
echo:
echo File "files_list_a.txt" created successfully.
echo Press any key to continue...
pause >nul
popd
exit /b
